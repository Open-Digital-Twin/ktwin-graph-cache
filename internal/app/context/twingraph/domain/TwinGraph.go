...
func (i TwinGraph) MarshalBinary() ([]byte, error) {
...
type TwinInstanceGraph interface {
	...
	PrintGraph()
	GetTwinGraph() TwinGraph
	GetTwinGraphByTwinInterfaces(twinInterfaces ...string) TwinGraph
	GetTwinInterfaces() []string
...
type twinInstanceGraph struct {
	NumberOfVertex   int
	TwinInterfaceMap map[string]bool
...
func (g *twinInstanceGraph) AddVertex(twinInstance dtdv0.TwinInstance) (*TwinInstanceGraphVertex, error) {
	...
	g.addTwinInterfaceMap(twinInstance.Spec.Interface)
...
func (g *twinInstanceGraph) addTwinInterfaceMap(twinInterface string) {
	g.TwinInterfaceMap[twinInterface] = true
}

func (g *twinInstanceGraph) GetTwinInterfaces() []string {
	var twinInterfaces []string
	for twinInterface := range g.TwinInterfaceMap {
		twinInterfaces = append(twinInterfaces, twinInterface)
	}
	return twinInterfaces
}
...
func (g *twinInstanceGraph) GetTwinGraph() TwinGraph {
...
func (g *twinInstanceGraph) GetTwinGraphByTwinInterfaces(twinInterfaces ...string) TwinGraph {
	var twinInstanceSettingsList []TwinInstanceEnvironmentSettings
	...
	for _, vertex := range g.Vertexes {
		...
		if slices.Contains(twinInterfaces, vertex.TwinInstance.Spec.Interface) {
			twinInstanceSettings := TwinInstanceEnvironmentSettings{
				Name:          vertex.TwinInstance.Name,
				Interface:     vertex.TwinInstance.Spec.Interface,
				Relationships: relationshipSettingList,
...
func (g *twinInstanceGraph) MarshalJson() ([]byte, error) {
	var twinGraph = g.GetTwinGraph()

	resultByte, err := json.Marshal(twinGraph)
